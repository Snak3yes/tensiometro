...
Responsabilidade: Criar 4 handlers (Keyboard, Menu, GRBL, DialogRouter).
...
class HandlersFactory:
    ...
    - Criar GRBLCallbackHandler (callbacks de GRBL)
    - Criar DialogRouter (rotas de diálogos)

    Methods:
    - create_all_handlers(): Cria todos os 4 handlers
    """
    ...
    def create_all_handlers(self, window: 'QWidget') -> Dict[str, Any]:
        ...
        # 3. GRBLCallbackHandler
        handlers['grbl_callback_handler'] = GRBLCallbackHandler(window)
        logger.debug("GRBLCallbackHandler criado via HandlersFactory")

        # 4. DialogRouter
        handlers['dialog_router'] = DialogRouter(window)
        logger.debug("DialogRouter criado via HandlersFactory")

        logger.info("Todos os 4 handlers criados via HandlersFactory")
